...
const API_BASE_URL = "https://duyduongth-toeicvocabulary.hf.space";
...
export const api = {
  ...
  updateVocabulary: async (
    vocabId: number,
    data: Partial<VocabIn>
  ): Promise<Vocabulary> => {
    const response = await axiosInstance.put(
      `/vocabulary/user/update/${vocabId}`,
      data
    );
    return response.data;
  },
  ...
  addVocabularyToUser: async (
    vocabId: number,
    masteryLevel: number = 0,
    notes?: string
  ): Promise<Vocabulary> => {
    const response = await axiosInstance.post(
      `/vocabulary/user/add/${vocabId}`,
      {
